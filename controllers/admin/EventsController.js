...
async function list(req, res) { 
    ...
    data = {};    
    action = 'list'; 
    const allRecord = await db.events.findAll({
...
async function edit(req, res) { 
    ...
    if(req.params.id){
        ...
        const entityDetail = await db.events.findOne({
            where:{
                id : id
            }
        });    
        ...
        if (req.method == "POST") {  
            ...
            if(errors){	   
                ...
                if (req.files && req.files.poster !== "undefined") { 
                    ...
                    poster.mv('public/upload/events/'+filename, function(err) { 
                        if (err){    
                            console.log(err);    
                            req.flash('error', 'Could not upload image. Please try again!')  
                            res.locals.message = req.flash();   
                            return res.redirect(nodeAdminUrl+'/Events/'+action); 
                        }     
                    }); 
                }  
                var msg =  controller+' updated successfully.';  
                input.id = req.params.id;
                var saveResult = await eventsService.updateEvents(input);  
            ...
            if (req.files && req.files.poster !== "undefined") { 
                ...
                poster.mv('public/upload/events/'+filename, function(err) { 
                    if (err){    
                        ...
                        req.flash('error', 'Could not upload image. Please try again!')  
                        res.locals.message = req.flash();   
                        return res.redirect(nodeAdminUrl+'/Events/add'); 
                    }     
                }); 
            }  
            // Save to DB 
            var saveResult=  await eventsService.addEvents(input);
            if(saveResult){    
...
async function deleteRecord(req, res) { 
    ...
    var categoryDetail = {}; 
    if(req.params.id){ 
        categoryDetail = await db.events.destroy({
            where: {
                id: req.params.id
            }
        });     
